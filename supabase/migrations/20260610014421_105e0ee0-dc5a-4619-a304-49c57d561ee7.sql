CREATE OR REPLACE FUNCTION public.get_referral_team(_referral_code text DEFAULT NULL)
RETURNS TABLE(
  id uuid,
  user_id uuid,
  email text,
...
BEGIN
  ...
  target_code := NULLIF(btrim(COALESCE(_referral_code, my_code)), '');

  IF target_code IS NULL THEN
    RETURN;
  END IF;

  IF target_code IS DISTINCT FROM my_code AND NOT public.has_role(auth.uid(), 'admin') THEN
    RETURN;
  END IF;
  ...
  WITH level_a AS (
    SELECT p.*
    FROM public.profiles p
    WHERE NULLIF(btrim(COALESCE(p.referred_by, '')), '') = target_code
  ),
  level_b AS (
    SELECT p.*
    FROM public.profiles p
    WHERE NULLIF(btrim(COALESCE(p.referred_by, '')), '') IN (
      SELECT a.referral_code FROM level_a a WHERE NULLIF(btrim(COALESCE(a.referral_code, '')), '') IS NOT NULL
    )
  ),
  level_c AS (
    SELECT p.*
    FROM public.profiles p
    WHERE NULLIF(btrim(COALESCE(p.referred_by, '')), '') IN (
      SELECT b.referral_code FROM level_b b WHERE NULLIF(btrim(COALESCE(b.referral_code, '')), '') IS NOT NULL
    )
  )
...
GRANT EXECUTE ON FUNCTION public.get_referral_team(text) TO anon, authenticated, service_role;
...
SET search_path TO 'public'
AS $$
  SELECT * FROM public.get_referral_team(NULL::text);
$$;

GRANT EXECUTE ON FUNCTION public.get_my_team() TO anon, authenticated, service_role;